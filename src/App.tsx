...
export default function App() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('local_empresas', JSON.stringify(empresas));
    } catch (e) {
      console.error("Erro ao sincronizar empresas:", e);
    }
  }, [empresas]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      <div className="p-6 max-w-7xl mx-auto">
        <header className="flex justify-between items-center pb-6 border-b border-slate-200 mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Gestor Consultor Pro</h1>
            <p className="text-xs text-slate-500 font-medium">Sistema de Gestão e Diagnósticos Empresariais</p>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" onClick={() => setView('home')}>Início</Button>
            <Button variant="outline" size="sm" onClick={() => setView('settings')}>Configurações</Button>
          </div>
        </header>
        ...
        <main>
          {view === 'home' && (
            <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-slate-600 text-sm">Estrutura carregada e corrigida com sucesso.</p>
            </div>
          )}
        </main>
      </div>
